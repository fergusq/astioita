...
def astias_index():
	return render_template("astias/list.html", astias=Astia.query.all(), statuses=Status.query.all(), users=User.query.all(), counts=User.find_astia_counts())

@app.route("/astiat/<astia_id>/", methods=["GET"])
def astias_show(astia_id: str):
	t: Astia = Astia.query.get(astia_id)
	return render_template("astias/show.html", astia=t, statuses=Status.query.all(), users=User.query.all())
